package com.ktpm.controller.nhankhau;
...
public class NhanKhauController implements Initializable {
	...
	@FXML
	private TableColumn<NhanKhau, String> hoVaTenColumn, biDanhColumn, ngaySinhColumn, cccdColumn, noiSinhColumn,
			gioiTinhColumn, nguyenQuanColumn, danTocColumn, noiThuongTruColumn, tonGiaoColumn, quocTichColumn,
			diaChiHienNayColumn, ngheNghiepColumn;
//    @FXML
//    private TableColumn<NhanKhau, Integer> idColumn;
	...
	public void initialize(URL location, ResourceBundle resources) {
		try {
			...
			while (result.next()) {
				nhanKhauList.add(new NhanKhau(result.getInt("ID"), result.getString("HoTen"),
						result.getString("BiDanh"), convertDate(result.getString("NgaySinh")), result.getString("CCCD"),
						result.getString("NoiSinh"), result.getString("GioiTinh"), result.getString("NguyenQuan"),
						result.getString("DanToc"), result.getString("NoiThuongTru"), result.getString("TonGiao"),
						result.getString("QuocTich"), result.getString("DiaChiHienNay"),
						result.getString("NgheNghiep")));
			}
		...
		if (soDu != 0)
			...
			pagination.setPageCount(nhanKhauList.size() / ROWS_PER_PAGE);
	...
	@FXML
	void dangKiTamTru(MouseEvent event) {
		try {
			Stage stage = new Stage();
			FXMLLoader fxmlLoader = new FXMLLoader(HomeApplication.class.getResource(TAM_TRU_FXML));
			Scene scene;
	...
	public Node createTableView(int pageIndex) {
		...
		indexColumn.setCellValueFactory(
				(Callback<TableColumn.CellDataFeatures<NhanKhau, NhanKhau>, ObservableValue<NhanKhau>>) p -> new ReadOnlyObjectWrapper(
						p.getValue()));
		...
		if (nhanKhauList.isEmpty())
			...
			if (lastIndex == pageIndex && displace > 0) {
				tableView.setItems(FXCollections.observableArrayList(
						nhanKhauList.subList(pageIndex * ROWS_PER_PAGE, pageIndex * ROWS_PER_PAGE + displace)));
			} else {
				tableView.setItems(FXCollections.observableArrayList(
						nhanKhauList.subList(pageIndex * ROWS_PER_PAGE, pageIndex * ROWS_PER_PAGE + ROWS_PER_PAGE)));
			}
		}
		return tableView;
	}
	...
	public void search() {
		...
		searchTextField.textProperty().addListener((observable, oldValue, newValue) -> {
			...
			pagination.setPageFactory(pageIndex -> {
				...
				indexColumn
						.setCellFactory(new Callback<TableColumn<NhanKhau, NhanKhau>, TableCell<NhanKhau, NhanKhau>>() {
							@Override
							public TableCell<NhanKhau, NhanKhau> call(TableColumn<NhanKhau, NhanKhau> param) {
								return new TableCell<NhanKhau, NhanKhau>() {
									@Override
